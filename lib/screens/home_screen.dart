...
class _HomeScreenState extends State<HomeScreen> {
  ...
  Future<void> _openForm([Aprendiz? aprendiz]) async {
    final changed = await Navigator.push<bool>(
      context,
      MaterialPageRoute(builder: (_) => AprendizFormScreen(aprendiz: aprendiz)),
    );
  ...
  Future<void> _delete(Aprendiz a) async {
    final confirmed = await showDialog<bool>(
      ...
      builder: (_) => AlertDialog(
        ...
        content: Text(
          '¿Está seguro de eliminar a ${a.nombre1} ${a.apellido1}? Esta acción no se puede deshacer.',
        ),
        actions: [
          TextButton(
    ...
    try {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Aprendiz eliminado correctamente.')),
      );
      _load();
    } catch (e) {
      if (!mounted) return;
                      ...
                      child: Padding(
                        ...
                        child: TextField(
                          ...
                          decoration: InputDecoration(
                            hintText: 'Buscar por nombre, correo, celular, departamento o ciudad...',
                            ...
                            children: [
                              ...
                              if (_aprendices.isEmpty) ...[
                                ...
                                FilledButton.icon(
                                  ...
                                  label: const Text('Registrar aprendiz'),
                                ),
                              ],
                            ],
                          ),
                            ...
                            return SingleChildScrollView(
                              ...
                              child: DataTable(
                                columns: const [
                                  ...
                                  DataColumn(label: Text('Aprendiz')),
                                ...
                                rows: _filtered.map((a) {
                                  return DataRow(cells: [
                                    DataCell(Text('${a.id ?? '-'}')),
                                    DataCell(Text('${a.nombre1} ${a.apellido1}')),
                                    DataCell(Text(a.celular)),
                                    DataCell(Text(a.email)),
                                    DataCell(Text(_depNames[a.departamento] ?? a.departamento)),
                                    DataCell(Text(_cityNames[a.ciudad] ?? a.ciudad)),
                                    DataCell(Row(
  ...
  Widget _mobileCard(Aprendiz a) {
    return ListTile(
      ...
      leading: CircleAvatar(
        child: Text(a.nombre1.isEmpty ? '?' : a.nombre1[0].toUpperCase()),
      ),
      title: Text('${a.nombre1} ${a.apellido1}'),
      subtitle: Text(
        '${a.email}\n${_cityNames[a.ciudad] ?? a.ciudad}, ${_depNames[a.departamento] ?? a.departamento}',
      ),
      isThreeLine: true,
      trailing: Wrap(
        spacing: 0,
        children: [
          IconButton(onPressed: () => _openForm(a), icon: const Icon(Icons.edit_outlined)),
          IconButton(
            onPressed: () => _delete(a),
            icon: const Icon(Icons.delete_outline),
            color: Colors.red,
